...
Texture2D LoadAndProcessLogo(const char *fileName) {
    ...
    if (!FileExists(targetFile)) {
        if (FileExists("images.png")) targetFile = "images.png";
        else if (FileExists("background.png")) targetFile = "background.png";
        else return (Texture2D){ 0 };
    }
...
// Load Player Character Picture with auto-white removal
Texture2D LoadAndProcessPlayer(const char *fileName) {
    const char *targetFile = fileName;
    if (!FileExists(targetFile)) {
        if (FileExists("character.png")) targetFile = "character.png";
        else if (FileExists("player.png")) targetFile = "player.png";
        else if (FileExists("character.jpg")) targetFile = "character.jpg";
        else return (Texture2D){ 0 };
    }
    ...
    // Optional: Make white or near-white background transparent
    for (int i = 0; i < pixelCount; i++) {
    ...
    while (!WindowShouldClose()) {
        ...
        BeginDrawing();
            ...
            if (state == STATE_MENU) {
                ...
                // Render Player Picture (or Fallback Circle)
                if (playerTexture.id > 0) {
                    float charSize = 42.0f; // Dimension of character picture
                    Rectangle src = { 0, 0, (float)playerTexture.width, (float)playerTexture.height };
                    
                    // Flip texture horizontally when moving left
